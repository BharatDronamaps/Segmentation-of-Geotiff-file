...
#imag = cv2.imread('Elevation_export.tif',0)
##imag = cv2.imread('Orthomosaic_export.tif')
##imag = cv2.imread('crop_img.png')
imag = cv2.imread('cropped1.tif')
##plt.imshow(imag)
...
##
img = morphology.remove_small_objects(img,min_size=500000,connectivity=2, in_place=True)
##plt.imshow(img)
...
##plt.show()
##cv2.waitKey(0)
cv2.imwrite('Python_Edges_elevation1.tif',edges)
...
cv2.imwrite('countour3.tif', imag)
...
cv2.drawContours(imag, contours, -1, (0,255,0), 3)
cv2.imwrite('countour1.tif', imag)
cnt = contours[0]
